                ...
                <td class="px-4 text-center">
                  <div class="btn-group" role="group">
                    <button
                      ...
                      data-bs-toggle="modal"
                      :data-bs-target="'#update' + drug._id"
                      data-modalLevel="add"
                    >
                      <i class="bi bi-pencil"></i>
                    </button>
                    <button
                      @click="deleteMedicine(drug._id)"
  ...
  try {
    const { data: responseData } = await useFetch(uri, {
      ...
      body: {
        ...
        how_to_use: newMedicine.value.guide,
        side_effect: newMedicine.value.sideEffect,
      },
    });
...
const deleteMedicine = async function (deletedID) {
  try {
    ...
    const { data: responseData } = await useFetch(`${uri}/${deletedID}`, {
      method: "delete",
    });
...
const updateMedicine = async function (drug) {
  try {
    const { data: responseData } = await useFetch(`${uri}/${drug._id}`, {
      method: "put",
      body: {
        id: drug._id,
        name: drug.name,
